...
def collect_activation_stats(model, tokenizer, texts, device, max_length=512):
    """
    Run `model` on `texts` and record per-layer input activation min/max.
    Returns: stats[name] = {'min': Tensor, 'max': Tensor}
    """
    model = model.to(device).eval()
    stats = {}
    handlers = []

    # Hook into each Linear to capture its input activations
    for name, module in model.named_modules():
        if isinstance(module, torch.nn.Linear):
            stats[name] = {'mins': [], 'maxs': []}
            def make_hook(n):
                def hook(mod, inputs, outputs):
                    x = inputs[0].detach().cpu()
                    stats[n]['mins'].append(x.amin(dim=0))
                    stats[n]['maxs'].append(x.amax(dim=0))
                return hook
            handlers.append(module.register_forward_hook(make_hook(name)))

    # Run calibration texts through model
    for txt in tqdm(texts, desc="Calibrating"):
        inputs = tokenizer(txt, return_tensors="pt",
                           truncation=True, max_length=max_length).to(device)
        with torch.no_grad():
            _ = model(**inputs)

    # Remove hooks and aggregate mins/maxs
    for h in handlers:
        h.remove()
    for name, s in stats.items():
        stats[name] = {
            'min': torch.stack(s['mins']).min(dim=0).values,
            'max': torch.stack(s['maxs']).max(dim=0).values,
        }
    return stats

def awq_quantize_block(weight_block, act_min=None, act_max=None, num_bits=4):
    """
    Block-wise uniform quantization stub (replace with activation-aware solver).
    ...
    q = torch.round(weight_block / scale + zero_point)\
             .clamp(qmin, qmax).to(torch.uint8)
    return q, scale, zero_point

def awq_quantize_model(model, activation_stats, num_bits=4, block_size=(128, 32)):
    """
    Quantize each Linear weight matrix in blocks.
    Returns: dict{name: [ {i,j,shape,q,scale,zero_point}, ... ]}
    """
    qstate = {}
    for name, param in model.named_parameters():
        ...
        # (we’re passing act_min/act_max but stub ignores them)
        for i in range(0, out_dim, b0):
            for j in range(0, in_dim, b1):
                wb = W[i:i+b0, j:j+b1]
                q, s, z = awq_quantize_block(wb, act.get('min'), act.get('max'), num_bits)
                blocks.append({
                    'i': i, 'j': j,
                    'shape': wb.shape,
                    'q': q, 'scale': s, 'zero_point': z
                })
        qstate[name] = blocks
    return qstate

def load_awq_model_from_state(model, awq_state):
    """
    Given a fresh model and `awq_state`, reconstruct and load each weight.
    """
    param_dict = dict(model.named_parameters())
    for name, blocks in awq_state.items():
        param = param_dict[name]
        W_recon = torch.zeros_like(param.data.cpu())
        for blk in blocks:
            i, j = blk['i'], blk['j']
            h, w = blk['shape']
            q = blk['q'].float()
            ...
            z = blk['zero_point']
            wb = (q - z) * s
            W_recon[i:i+h, j:j+w] = wb
        param.data.copy_(W_recon.to(param.device))
    return model
